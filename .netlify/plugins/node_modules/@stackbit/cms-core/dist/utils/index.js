        ...
        if (fileStat.isDirectory()) {
            ...
            if (outputPathObject.ext === '.njk') {
                if (!_.has(options, 'processNunjucksFile') || !_.isFunction(options.processNunjucksFile)) {
                    throw new Error(`utils.copyFilesRecursively(): file (${sourceFilePath}) has '.njk' extension but processNunjucksFile function was not passed`);
                }
                const relativeSourceFilePath = path.relative(_internalOptions.origSourceDir, sourceFilePath);
            ...
            const relativeTargetFilePath = path.relative(_internalOptions.origTargetDir, targetFilePath);
...
function readDirGlob(dir, options) {
    ...
    return new Promise((resolve, reject) => {
        glob(dirPattern, _.get(options, 'globOptions', {}), (err, files) => {
            ...
            resolve(_.compact(files.map((file) => {
...
function fromPath(pathArray) {
    return _.reduce(pathArray, (accum, pathPart) => {
