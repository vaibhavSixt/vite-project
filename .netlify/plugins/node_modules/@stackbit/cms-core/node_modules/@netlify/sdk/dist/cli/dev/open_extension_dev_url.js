...
export async function openExtensionDevUrl(extensionDevUrl, slug) {
    const netlifyCliUserAuthToken = await getToken();
    if (!netlifyCliUserAuthToken?.[0]) {
        console.error("You are not logged in to Netlify. Please run `netlify login` to use UNSTABLE__DEV_OPEN.");
        return;
    }
    ...
    const res = await fetch(`${appOrigin}/.netlify/functions/ntli-extension-dev-auth`, {
        ...
        body: JSON.stringify({
            slug: process.env.UNSTABLE__DEV_SLUG_OVERRIDE || slug,
            url: extensionDevUrl,
            redirect_path: process.env.UNSTABLE__DEV_REDIRECT_PATH,
        }),
    });
    if (!res.ok) {
    ...
    open(url.toString(), { wait: true }).catch(() => {
        ntliLog(`Failed to open Extension UI. You can manually navigate to ${url.toString()} in your browser.`);
    });
}
