...
const { utils, consts } = require('@stackbit/cms-core');
module.exports = {
    loadContent,
    addMetadata
};
async function loadContent({ dirPath, schema }) {
    ...
    excludePages.push(...consts.LOADER_EXCLUDED_FILES);
...
async function loadConfigFile(configFiles, absProjDirPath, schema, modelsByName) {
    ...
    const configFilePath = await utils.getFirstExistingFile(configFiles, absProjDirPath);
    if (!configFilePath) {
        return { data: null, errors: [] };
    }
...
async function loadData({ dirPath, schema, modelsByName, excludeFiles }) {
    ...
    return utils.dataReducer(sortedFilePaths, async (relFilePath) => {
...
async function loadPages({ dirPath, schema, modelsByName, excludeFiles }) {
    ...
    return utils.dataReducer(sortedFilePaths, async (relFilePath) => {
        ...
        try {
            const model = matchObjectToModel(result.data, pageModels, relFilePath, pageLayoutKey, ['layout']);
...
async function readDir(absModelDirPath, excludeFiles) {
    const modelRelFilePaths = await utils.readDirRec(absModelDirPath, {
        filter: (filePath, stats) => !_.some(excludeFiles, (exclude) => micromatch.isMatch(filePath, exclude)) &&
            (!stats.isFile() || ['md', 'markdown', 'mdx', 'yml', 'yaml', 'json', 'toml'].includes(path.extname(filePath).substring(1)))
    });
...
function parseFileWithErrors(filePath) {
    return utils
        ...
        if (['md', 'markdown', 'mdx'].includes(extension) && _.has(data, 'frontmatter') && _.has(data, 'markdown')) {
...
function addMetadata(data, model, { filePath, modelsByName, dataFieldPath = [], modelFieldPath = null }) {
    ...
    data = _.assign(data, {
        __metadata: _.omitBy({
            srcObjectId: dataFieldPath.length === 0 ? utils.convertToPosixFilePath(filePath) : null,
    ...
    return utils.dataReducerSync(data, (fieldValue, fieldName) => {
        ...
        if (fieldModel.type === 'list') {
            ...
            if (['object', 'model', 'reference'].includes(itemsType)) {
                ...
                return utils.dataReducerSync(fieldValue, (item, idx) => {
...
function addMetadataByField(fieldValue, fieldModel, context) {
    ...
    if (fieldType === 'object') {
        ...
        if (!modelNames || !_.isArray(modelNames) || modelNames.length < 1) {
            return {
                data: fieldValue,
                errors: [`Field of type 'model' must have a 'models' property with an array of at least one model name, ${positionInSchema(context)}`]
            };
        }
        if (modelNames.length === 1) {
            ...
            if (!_.has(modelsByName, modelName)) {
                return {
                    data: fieldValue,
                    errors: [`The 'models' property of a field of type 'model' must reference an existing model, ${positionInSchema(context)}`]
                };
            }
            return addMetadata(fieldValue, modelsByName[modelName], { ...context, modelFieldPath: [modelName] });
        }
        if (!_.has(fieldValue, 'type')) {
            return {
                data: fieldValue,
                errors: [`The object referenced by a field of type 'model' with multiple models must specify the 'type' property, ${positionInData(context)}`]
            };
        }
        if (!_.has(modelsByName, fieldValue.type)) {
            return {
                data: fieldValue,
                errors: [`The 'type' property of an object referenced by a field of type 'model' must reference an existing model, ${positionInData(context)}`]
            };
        }
        return addMetadata(fieldValue, modelsByName[fieldValue.type], { ...context, modelFieldPath: [fieldValue.type] });
    }
