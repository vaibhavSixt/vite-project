...
type StringFieldTypes = Exclude<FieldType, 'number' | 'boolean' | 'enum' | 'object' | 'model' | 'reference' | 'cross-reference' | 'style' | 'list'>;
...
export async function generateSchema({ ssgMatchResult, ...fileBrowserOptions }: SchemaGeneratorOptions): Promise<SchemaGeneratorResult> {
    ...
    const objectModels: ObjectModel[] = _.map(dataModelsResults.objectModels, (objectModel, index): ObjectModel => {
        ...
        switch (type) {
            ...
            case 'object': {
                ...
                if (result.fieldsList.length === 1) {
                    ...
                    const models = result.fieldsList.map((fields): PartialObjectModel => {
                        const modelName = generateRandomModelName();
                        return {
                            type: 'object',
                            name: modelName,
...
function analyzePageFileMatchingProperties(partialPageModels: PartialPageModelWithFilePaths[]) {
    let pageCount = 1;
    partialPageModels = _.map(partialPageModels, (partialPageModel: PartialPageModelWithFilePaths): PartialPageModelWithFilePaths => {
