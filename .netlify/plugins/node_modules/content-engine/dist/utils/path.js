...
const withBasePath = basePath => (...paths) => (0, _coreUtils.joinPath)(basePath, ...paths);
...
const truncatePath = path => path.replace(pathSegmentRe, match => {
  if (isNameTooLong(match)) {
    return match.slice(0, SLICING_INDEX) + (0, _coreUtils.createContentDigest)(match.slice(SLICING_INDEX));
  }
  return match;
});
