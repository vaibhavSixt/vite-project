...
function get(obj, selectors, options) {
  ...
  const value = obj[key];
  if (!value && options !== null && options !== void 0 && options.keepNonMatches) {
    return obj;
  }
...
function getArray(arr, selectors, options) {
  return arr.map(value => Array.isArray(value) ? getArray(value, selectors, options) : get(value, selectors, options)).filter(v => v !== undefined);
}
