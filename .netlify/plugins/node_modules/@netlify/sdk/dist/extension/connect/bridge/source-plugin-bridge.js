...
export class SourcePluginBridgeAPI extends BridgePluginAPI {
    ...
    createResolvers({ createResolvers, emitter }) {
        ...
        if (Object.keys(resolverDefinitions).length > 0) {
            emitter.emit("LEDGER_DEPENDENCY_MANAGER_SHOULD_INSTALL_DYNAMIC_CONNECTOR", {
                payload: {
                    plugin: { name: this.runningConnector.slug },
                },
            });
    ...
    `);
        ...
        // cloud-sourcerer still uses Gatsby so the custom directives from content-engine aren't present but we need it to exist
        // and another connector instance hasn't already defined it
        !globalThis.___custom_directives_added) {
            // then define the authorization directive
            globalThis.___custom_directives_added = true;
            schemaCustomizationApi.store.dispatch({
                type: `CREATE_FIELD_EXTENSION`,
                payload: {
                    name: "authorization",
