...
class LedgerDependencyManager {
    ...
    async installPendingPlugins() {
        ...
        const pnpmArgs = this.getPnpmInstallArguments();
        ...
        if (pnpmArgs.length > 0) {
            if (this.sawAtleastOneGatsbyPlugin) {
                ...
                pnpmArgs.push(`gatsby@link:${thisContentEnginePath}`);
            }
            console.info(`Running "pnpm install ${pnpmArgs.join(` `)}" in ${this.directory}`);
            await (0, exec_1.default)([`install`, ...pnpmArgs], {
                cwd: this.directory,
            });
            console.info(`Finished installing ledger plugins`);
        }
        return { totalInstalled: pnpmArgs.length, previouslyInstalled: 0 };
    }
    ...
    getPnpmInstallArguments() {
        ...
        return [...pendingInstallArgs, `link:${thisGraphQLPath}`];
    }
