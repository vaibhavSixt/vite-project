...
function makeLedgerWriteHandler({ getWritableStream, }) {
    return async (req, res) => {
        const ledgerId = parseStringHeader(req.headers, `x-ledger-id`);
        const configurationId = parseStringHeader(req.headers, `x-configuration-id`);
        const cacheId = parseStringHeader(req.headers, `x-cache-id`);
        const blockId = parseStringHeader(req.headers, `x-block-id`);
