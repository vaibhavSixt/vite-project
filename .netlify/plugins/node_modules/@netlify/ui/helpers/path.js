import { ENTERPRISE_SLUG } from "./plans.js";
...
function devServerPathWithSite(site, devServer) {
  if (!(devServer == null ? void 0 : devServer.id)) {
    return sitePath(site, "dev-servers");
  }
  return sitePath(site, "dev-servers", devServer.id);
}
function devServerPath(devServer) {
  return ["/sites", devServer.site_id, "dev-servers", devServer.id].join("/");
}
...
function enterpriseUpgradePath(account) {
  const params = new URLSearchParams({ plan: ENTERPRISE_SLUG });
...
export {
  ...
  deploysPath,
  devServerPath,
  devServerPathWithSite,
  dnsPath,
  docsPath,
  docsPathWithHash,
  edgeFunctionsPath,
  edgeFunctionsPathWithTimeRangeAndBranch,
  enterpriseUpgradePath,
  formPath,
  formsPath,
  forumsPath,
  functionPath,
  functionPathWithTimeRangeAndBranch,
  functionsPath,
  getLastSegmentOfPath,
  getPathSegments,
  getPathSegmentsWithoutParams,
  integrationsPath,
  membersBuildPath,
  membersPath,
  organizationPath,
